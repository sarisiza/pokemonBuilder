...
class GetTypesListUseCase @Inject constructor(
    ...
    ): Flow<UIState<List<TypesQuery.Pokemon_v2_type>>> = flow {
        emit(UIState.LOADING)
        serviceCall.serviceCallApi.apolloServiceCall(
            {networkRepository.getTypes(language.language)},
            {emit(UIState.SUCCESS(it.pokemon_v2_type))},
